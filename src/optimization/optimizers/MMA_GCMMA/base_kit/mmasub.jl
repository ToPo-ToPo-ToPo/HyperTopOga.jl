...
#
#
function mmasub(m,n,iter,xval,xmin,xmax,xold1,xold2,df0dx,fval,dfdx,low,upp,a0,a,c,d,asyinit,asyinc,asydec,move)
    #
    ...
    if iter < 2.5
        ...
        factor = copy(eeen)
        factor[findall(x -> x>0, zzz)] .= asyinc
        factor[findall(x -> x<0, zzz)] .= asydec
