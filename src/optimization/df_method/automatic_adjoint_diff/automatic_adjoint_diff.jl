...
# 
#-------------------------------------------------------------------------------------------------
struct AutomaticAdjointDiff <: AbstractDfMethod 
    name::String
    #---------------------------------------------------
    #
    #---------------------------------------------------
    function AutomaticAdjointDiff()
        return new("Automatic_adjoint_diff")
    end 
end
#-------------------------------------------------------------------------------------------------
# 自動微分による感度解析
...
function compute_f_and_grad(
    ...
    df = compute_grad_by_automatic_adjoint(eval, physics, opt_settings, opt_filter)
